...
#
# For each combo the script updates .env, restarts vLLM via docker compose,
# waits for the health check, then runs locust at every user level (nothink
# always; think + mixed when MAX_MODEL_LEN is large enough for reasoning chains).
...
#
# Prerequisites:
#   - OPENWEBUI_API_KEY and OPENWEBUI_KB_ID in .env
...
MAX_MODEL_LENS=(32768)
MAX_NUM_SEQS_LIST=(32)
...
restart_vllm() {
    ...
    log "Updating .env: VLLM_MAX_MODEL_LEN=$max_len  VLLM_MAX_NUM_SEQS=$max_seqs"
    sed -i "s/^VLLM_MAX_MODEL_LEN=.*/VLLM_MAX_MODEL_LEN=$max_len/" .env
    sed -i "s/^VLLM_MAX_NUM_SEQS=.*/VLLM_MAX_NUM_SEQS=$max_seqs/" .env
    log "Restarting vLLM container..."
    docker compose up -d vllm --force-recreate 2>&1 | grep -v "^$" || true
    # Wait for the health endpoint to respond
    local url="${VLLM_HEALTH_URL:-http://localhost:8000/health}"
    log "Polling $url until vLLM is healthy..."
...
run_combo() {
    ...
    if $DRY_RUN; then
        log "[dry-run] would restart vLLM with mlen=$max_len seqs=$max_seqs"
    else
        restart_vllm "$max_len" "$max_seqs"
    fi
