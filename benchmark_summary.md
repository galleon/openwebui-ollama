...
- **Remaining bottleneck**: at 100 users even with 64 slots, ~36 users still queue. True 100-user concurrency at acceptable latency likely requires a second GPU instance.
...
| Option | Description | Expected impact |
|---|---|---|
| Second GPU + load balancer | Second RTX Pro 6000 running an identical stack; nginx/Traefik round-robins between them. `docker-compose.rtx6000.yml` already documents the two-stack setup. | Linear capacity scaling — halves latency and doubles throughput at 100 users. |
...
3. If latency targets are still not met at 100 users, add a second GPU instance — it is the only path to <10s RAG TTFT at that concurrency on this model.
